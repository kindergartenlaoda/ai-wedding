...
export async function GET(req: NextRequest) {
  ...
  if (isLocalModelConfigStoreEnabled()) {
    ...
    const maskedData = configs.map(withMaskedApiKey);
    return NextResponse.json({ data: maskedData, total: maskedData.length, local: true });
  }
...
export async function POST(req: NextRequest) {
  ...
  if (isLocalModelConfigStoreEnabled()) {
    const config = await createLocalModelConfig(body, authResult.profile.user_id);
    return NextResponse.json({ data: withMaskedApiKey(config), local: true }, { status: 201 });
  }
