...
export async function GET(req: NextRequest) {
  ...
  if (isLocalAdminStoreEnabled()) {
    ...
    return NextResponse.json({ data: domains, total: domains.length, local: true });
  }
...
export async function POST(req: NextRequest) {
  ...
  if (isLocalAdminStoreEnabled()) {
    const domain = await createLocalDomain(body);
    if (domain === 'duplicate') {
      return NextResponse.json({ error: `slug "${body.slug}" already exists` }, { status: 409 });
    }
    return NextResponse.json({ data: domain, local: true }, { status: 201 });
  }
