    ...
    <div className="p-8 mb-8 rounded-sm border shadow-inner bg-black/40 border-white/10">
      ...
      <div className="mb-8">
        <label className="block flex gap-3 items-center mb-3 text-xs font-medium text-alabaster uppercase tracking-widest">
          {selectedTemplate ? '附加场景与要求' : '定制光影提示词'}
          {selectedTemplate && (
            <span className="text-xs font-light text-pearl/60 tracking-wide normal-case">（与当前模板叠加）</span>
          )}
        </label>
        <textarea
          value={customPrompt}
          ...
          placeholder={selectedTemplate
            ? '例如：巴黎街头雨夜、暖色电影光、全身构图；保留模板服装与人物五官'
            : '例如：电影感海边日落人像，柔和粉色天空，全身构图'}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => onSettingsChange({
              ...settings,
              imageQuality: e.target.value as ImageGenerationSettings['imageQuality']
            })}
            className="px-5 py-4 w-full rounded-sm border transition-all border-white/10 focus:ring-1 focus:ring-gold focus:border-gold bg-black text-alabaster font-light outline-none cursor-pointer tracking-wider text-sm"
          >
            <option value="high">高清细节（推荐）</option>
            <option value="medium">标准质量</option>
            <option value="low">快速预览</option>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => onSettingsChange({
              ...settings,
              imageSize: e.target.value as ImageGenerationSettings['imageSize']
            })}
            className="px-5 py-4 w-full rounded-sm border transition-all border-white/10 focus:ring-1 focus:ring-gold focus:border-gold bg-black text-alabaster font-light outline-none cursor-pointer tracking-wider text-sm"
          >
            <option value="1024x1536">竖版高清 1024 × 1536</option>
            <option value="1536x1024">横版高清 1536 × 1024</option>
