...
[![Platform](https://img.shields.io/badge/Platform-Windows%20%7C%20macOS%20%7C%20Linux-success)](https://github.com/kindergartenlaoda/ai-wedding)
...
该模式无需 PostgreSQL、MinIO、注册登录或积分系统。本地会自动初始化完整模板、领域、项目和生成记录，数据保存在不会提交到 Git 的 `.local/` 目录。
...
```bash
# macOS / Linux
cp .env.local-admin.example .env

# Windows PowerShell
Copy-Item .env.local-admin.example .env
```

编辑 `.env`，至少填写 `IMAGE_API_BASE_URL`、`IMAGE_API_KEY` 和 `IMAGE_IMAGE_MODEL`，然后启动：

```bash
pnpm dev
```

访问 [http://127.0.0.1:3000](http://127.0.0.1:3000)。本地管理员模式默认启用全部模板、最多 8 张批量生成、高清输出和后台配置。
...
- **Bug 报告**: [Issues](https://github.com/kindergartenlaoda/ai-wedding/issues)
- **功能建议**: [Issues](https://github.com/kindergartenlaoda/ai-wedding/issues)
- **讨论交流**: [Discussions](https://github.com/kindergartenlaoda/ai-wedding/discussions)
